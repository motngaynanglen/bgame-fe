...
function TimeSlotDisplay({ storeid, date }: { storeid: string, date?: Date }) {
    ...
    const timeSlotStyle = (value: number): CSSProperties => ({
        ...
        borderRadius: '4px',
        backgroundColor: value === 1 ? 'rgba(255, 77, 79, 0.2)' : 'rgba(102, 187, 106, 0.2)',
                ...
                <div style={containerStyle}>
                    ...
                    {timeSlots.map((slot, index) => (
                        <React.Fragment key={`marker-${index}`}>
                            ...
                            <div style={{ ...timeMarkerStyle, left: `${(index + 1) * 80}px` }}>
                                {`${slot.end}h`}
                            </div>
                        </React.Fragment>
                    ))}
