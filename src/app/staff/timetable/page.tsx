...
import { Breadcrumb, Button, Card, Col, Collapse, DatePicker, Dropdown, message, Modal, Pagination, Radio, Row, Select, Space, Table, Tag, TimePicker } from "antd";
...
export default function StaffManageTimeTable({ searchParams }: { searchParams?: { query?: string; page?: string; }; }) {
    ...
    const handleDateChange = (date: dayjs.Dayjs | null) => {
        if (date) {
            const from = date.set("hour", 1).set("minute", 0).set("second", 0).toISOString();
    ...
    const handleRangeChange = (dates: [dayjs.Dayjs | null, dayjs.Dayjs | null] | null) => {
        if (dates && dates[0] && dates[1]) {
            const from = dates[0].set("hour", 1).set("minute", 0).set("second", 0).toISOString();
    ...
    const columns: TableProps<DataType>['columns'] = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <>
                    {formatDateTime(record.from, "DATE")}
                </>
            ),
        },
        {
            ...
            render: (_, record) => (
                <>
                    {formatDateTime(record.from, "TIME")}
                </>
            ),
        },
        {
                        ...
                        <Col span={12} className="flex justify-center">
                            {(() => {
                                switch (record.status) {
                                    case "ACTIVE":
                                        return <Button color="primary" variant="dashed" onClick={() => onClickStart(record.id)}>Check</Button>;
                                    case "STARTED":
                                        return (
                                            <Space className="flex justify-between">
                                                <Button color="cyan" variant="filled" onClick={() => onClickEnd(record.id)}>End</Button>
                                                <Button color="yellow" variant="filled" onClick={() => setOpenModal(true)}>Extend</Button>
                                            </Space>
                                        );
                                    case "ENDED":
                                        return <Button variant="filled" disabled>DONE</Button>;
                                    default:
                                        return null;
                                }
                            })()}
