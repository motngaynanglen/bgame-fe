...
import { BookingRequestBody, ProductViewModel } from "./types";
...
export default function BookingPaymentModal({
  ...
  const createBooking = useMutation({
    mutationFn: async (data: BookingRequestBody) => {
      const res = await bookListApiRequest.createBookListByStaff(data, user?.token);
  ...
  const fromSlot = bookingBody?.fromSlot ?? 0
  const toSlot = bookingBody?.toSlot ?? 0
  const bookDate = bookingBody?.bookDate ?? ""
  ...
  const totalAmount = products.reduce((sum, p) => sum + (p.rent_price_per_hour ?? 0), 0) * totalHours;
                  ...
                  renderItem={(item) => (
                    <List.Item>
                      <List.Item.Meta
                        ...
                        title={<div className="font-medium">{item.product_name}</div>}
                        description={
                          <>
                            <Text type="secondary">
                              {item.rent_price_per_hour?.toLocaleString()} đ/giờ
                            </Text>
                            <div style={{ color: "#fa541c" }}>
                              {((item.rent_price_per_hour ?? 0) * totalHours).toLocaleString()} đ
                            </div>
                          </>
                        }
                      />
              ...
              {user && isAuthenticated ? (
                <>
                  <Button
                    ...
                    disabled={paymentData?.id != undefined}
                  >
        ...
        <div className="text-center py-6">
          ...
          <Button type="primary" onClick={() => {
            if (paymentData && paymentData.id) {
              handlePaymentAction(paymentData.id)
            } else {
