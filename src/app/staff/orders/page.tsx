...
/** Main component */
export default function OrderHistoryStaffPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res: HistoryResponse = await orderApiRequest.getOrderHistoryPaged(
        body,
        user?.token
      );
      if (!(res.statusCode === "200"))
        throw new Error(`HTTP ${res.statusCode}: ${res.message}`);
      setData(res.data || []);
      // Server returns TotalCount inside first row or use paging fallback
      const totalFromRow = res.data?.[0]?.TotalCount;
      const totalFromPaging = res.paging
        ? res.paging.pageCount * res.paging.pageSize
        : undefined;
  ...
  const handlePay = (order: OrderGroup) => {
    Modal.confirm({
      ...
      content: (
        <>
          <p>
            Đơn hàng: <b>#{order.code}</b>
          </p>
          <p>
            Ngày đặt: <b>{formatDateTime(order.created_at, "DATE")}</b>
          </p>
          <p>
            Tổng tiền: <b>{formatVND(order.total_price)}</b>
          </p>
        </>
      ),
      okText: "Thanh toán",
  ...
  const columns: ColumnsType<OrderGroup> = [
    ...
    },
    {
      ...
      render: (v: boolean, r) => (
        <Space direction="vertical" size={0}>
          <Tag color={v ? "processing" : "default"}>
            {v ? "Giao tận nơi" : "Nhận tại quầy"}
          </Tag>
          {v && r.delivery_brand && (
  ...
  return (
    <>
      <div style={{ display: "grid", gap: 12 }}>
        ...
        <Text type="secondary">
          Xem các đơn hàng theo nhóm (GroupOrder). Nhấp để xem chi tiết từng cửa
          hàng và sản phẩm.
        </Text>
